...
const addPost = catchError(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if(req.file)req.body.content = req.file?.filename;
...
const getAllPosts = catchError(
  async (
    ...
    let apiFeatuers = new ApiFeatuers(
      Post.find({ finished: true })
        .populate({
...
const getUnFinishedPosts = catchError(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
...
const editPost = catchError(
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    if (req.file) removeOldImage(post?.content, "posts");
